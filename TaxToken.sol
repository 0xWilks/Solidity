...
contract TaxToken is ERC20, Ownable {
    ...
    constructor(address _fundAddress) ERC20("TaxToken","Tax") {
        _mint(msg.sender, 5000000 * 10 ** decimals());
        fundAddress = _fundAddress;
        isWhitelisted[fundAddress] = true;
    }

    function getLatestPrice(address _priceFeedAddress) external view returns (int256) {
    ...
    function updateFund(address payable fund) public onlyOwner() {
        fundAddress = fund;
        isWhitelisted[fundAddress] = true;
    }
    ...
    function rescueETH(uint256 etherAmount) public onlyOwner {
        uint256 weiAmount = etherAmount * 10**18;
    ...
    function _transfer(
        ...
        } else if (recipient == uniswapPair && !isWhitelisted[sender]) {
